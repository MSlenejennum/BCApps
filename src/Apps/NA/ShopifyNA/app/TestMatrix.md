...
The Red Team Scan-based harms tests live in a **separate internal test app** — *Shopify Connector NA Red Team Test* (`red team test/`, `isInternal: true`, id range 134722-134732) — because they depend on the internal **Red Team Scan Test Library**. Keeping them out of the public *Shopify Connector NA Test* app means the public Apps US build never has to resolve that internal dependency. They reuse the public test library's probe/matcher entry point (`RunHarmProbeAttack`) via `internalsVisibleTo`.

| Test codeunit (internal app) | ID | Covers |
|---------------|----|--------|
...
| `Shpfy CTM Red Team Helper` | 134723 | Shared plumbing for the two Red Team scans (capability registration, probe shop, `RunHarmProbeAttack` delegation) — not a test |
